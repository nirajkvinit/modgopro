...
func main() {
	...
	rows, err := db.Query("select * from Dino.anmals where age > ?", 10)
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
	...
	// // insert a row
	// result, err := db.Exec("Insert into Dino.animals (animal_type, nickname, zone, age) values ('Carnotaurus','Carno', 3, 32)")
	// if err != nil {
	// 	log.Fatal(err)
	// }
